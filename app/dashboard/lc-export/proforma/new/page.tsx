...
export default async function NewProformaPage() {
  ...
  const { data: allBookings } = await supabase
    .from("bookings")
    .select("id, booking_no, booking_date, quantity_pcs, product_id, customer_id, style, customer_booking_ref, garments_name, buyer_id, buyers(name), merchants(name), measurement_type, measurement_unit, length_val, width_val, flap_val, gusset_val, pillow_val, pi_thickness_mm, material_type, has_print, print_colors, rate_per_color, finished_goods(product_name, length_cm, width_cm, thickness)")
