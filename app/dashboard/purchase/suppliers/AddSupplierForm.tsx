...
export default function AddSupplierForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    setLoading(true);
    const { error } = await supabase.from("suppliers").insert({ name, address, phone, email });
    setLoading(false);
    if (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border bg-white p-4 shadow-sm mb-6 space-y-3">
      <h2 className="font-semibold text-gray-800">নতুন Supplier যোগ করুন</h2>
      <div className="flex flex-wrap gap-3">
        ...
        <input placeholder="ফোন" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-40 rounded-lg border px-3 py-2 text-sm" />
