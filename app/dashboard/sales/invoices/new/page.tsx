...
export default async function NewSalesInvoicePage() {
  ...
  const { data: bookings } = await supabase
    .from("bookings")
    .select("id, booking_no, quantity_pcs, product_id, customer_id, style, garments_name, buyers(name), merchants(name), delivery_point, customer_booking_ref, has_print, print_colors, rate_per_color, rate_per_inch, measurement_type, measurement_unit, length_val, width_val, flap_val, gusset_val, thickness_mm, finished_goods(product_name, length_cm, width_cm, thickness)")
