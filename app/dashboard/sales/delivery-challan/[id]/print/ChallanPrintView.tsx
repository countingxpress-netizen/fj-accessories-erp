...
export default function ChallanPrintView({
  challan, company, challanDateLabel,
  ...
  // এক পেজে না আঁটলে (আনুমানিক A4 উচ্চতা) নন-প্রিন্ট ওয়ার্নিং — re-render ছাড়াই DOM-এ
  useEffect(() => {
    if (sheetRef.current && warnRef.current && sheetRef.current.scrollHeight > 1050) {
  ...
  return (
    <div className="mx-auto max-w-[210mm] bg-white text-gray-900 challan-root">
      ...
      <div ref={sheetRef} className="challan-sheet">
        ...
        <table className="mb-6 w-full border-collapse text-sm">
          <thead>
            <tr className="border-b-2 border-gray-800">
              ...
              <th className="w-40 py-1.5 text-right">Quantity</th>
              {hasPackets && <th className="w-28 py-1.5 text-right">Packets</th>}
            </tr>
          </thead>
          <tbody>
            {items.map((item) => {
        ...
        <p className="mb-16 text-sm">Received the above goods as per order with good condition.</p>
